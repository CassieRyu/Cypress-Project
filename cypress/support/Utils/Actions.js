export const getRandomValueFromDropdownList = function () {
  return new Cypress.Promise((resolve, reject) => {
    cy.get('div[class*="dropdown__option"]').then(($option) => {
      ...
      cy.log(random);
      cy.get('div[class*="dropdown__option"]')
        .eq(random)
        .invoke('text')
        .then(($text) => {
          resolve($text);
...
export const selectDropdownList = function (labelText, valueText, parentCss) {
  return new Cypress.Promise((resolve, reject) => {
    ...
    cy.get(parentCss).within(() => {
      cy.contains('div', labelText, { timeout: 10000 })
        .next()
        .find('div[class*="dropdown__control"]')
        .find('input')
        .focus()
        .type(' ', { force: true })
        .then(() => {
          getRandomValueFromDropdownList(parentCss).then(($random) => {
            ...
            cy.get('div[class*="dropdown__menu-list"]')
              .contains('div', valueText)
              .click();
            resolve(valueText);
          });
        });
